"""Logistics bucket calculation: warehousing / last-mile transport bucket.

Formula per docs/CARBON_ENGINE.md:
Let t = material_quantity_kg / 1000 (tonnes of goods)
logistics_co2e_kg = (t * 50) * factor_kg_co2e_per_unit
Assumed 50 km last-mile distance.
Factor code: logistics_road
Unit: tonne_km
"""

from typing import Optional

from ..constants import DEFAULT_LAST_MILE_KM, EmissionCategory, FactorCategory, LOGISTICS_ROAD_FACTOR_CODE
from ..factors import FactorLookupResult, FactorRegistry, get_default_registry
from ..models import AuditBucketDetail
from ..units import calculate_tonne_km, kg_to_tonnes


def calculate_logistics_emissions(
    material_quantity_kg: float,
    last_mile_km: float = DEFAULT_LAST_MILE_KM,
    org_id: Optional[str] = None,
    registry: Optional[FactorRegistry] = None,
) -> tuple[float, AuditBucketDetail]:
    """Calculate last-mile logistics emissions in kg CO2e."""
    if registry is None:
        registry = get_default_registry()

    lookup: FactorLookupResult = registry.get_with_fallback(
        category=FactorCategory.LOGISTICS,
        code=LOGISTICS_ROAD_FACTOR_CODE,
        org_id=org_id,
    )

    tonnes = kg_to_tonnes(float(material_quantity_kg))
    tonne_km = calculate_tonne_km(float(material_quantity_kg), float(last_mile_km))
    co2e_kg = round(tonne_km * lookup.value, 4)

    formula_str = (
        f"({tonnes} tonnes * {last_mile_km} km last-mile = {tonne_km} tonne_km) * "
        f"{lookup.value} kg CO2e/tonne_km"
    )

    audit = AuditBucketDetail(
        category=EmissionCategory.LOGISTICS,
        activity_value=round(tonne_km, 4),
        activity_unit="tonne_km",
        factor_code=lookup.code,
        factor_value=lookup.value,
        factor_unit=lookup.unit,
        formula=formula_str,
        co2e_kg=co2e_kg,
        fallback_applied=lookup.is_fallback,
        fallback_note=lookup.fallback_note,
    )

    return co2e_kg, audit

